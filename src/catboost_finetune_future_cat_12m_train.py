...
if __name__ == "__main__":
    ...
    train_updrs_1_cat_path = os.path.join(
        "..", "data", "processed", "train_updrs_1_cat.csv"
    )
    train_updrs_2_cat_path = os.path.join(
        "..", "data", "processed", "train_updrs_2_cat.csv"
    )
    train_updrs_3_cat_path = os.path.join(
        "..", "data", "processed", "train_updrs_3_cat.csv"
    )
    # read in the protein and updrs data
    updrs1_df = pd.read_csv(train_updrs_1_cat_path)
    updrs2_df = pd.read_csv(train_updrs_2_cat_path)
    updrs3_df = pd.read_csv(train_updrs_3_cat_path)
    ...
    for updrs, df in zip(
        ...
        save_catboost_hyperparams_path = os.path.join(
            ".", "models", f"catboost_12m_hyperparam_finetune_results_{updrs}.csv"
        )
        final_results = pd.DataFrame(test_params_dict)
        test_params_dict = dict()
        final_results.to_csv(save_catboost_hyperparams_path, index=True)
