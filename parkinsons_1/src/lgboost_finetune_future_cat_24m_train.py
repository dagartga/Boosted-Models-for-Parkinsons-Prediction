...
if __name__ == "__main__":
    ...
    val_list = tree_learner + boosting + feature_fraction + is_unbalance
    param_names = (
    ...
    for updrs, df in zip(
        ...
        for param, val in zip(param_names, val_list):
            ...
            pd.DataFrame(test_params_dict)

        # save the results
        final_results = pd.DataFrame(test_params_dict)
        final_results.to_csv(
            f"./models/lgboost_24m_hyperparam_finetune_results{updrs}.csv", index=True
        )
