...
def run(model, target, month_diff):
    ...
    df = pd.read_csv(
        f"~/parkinsons_proj_1/parkinsons_project/parkinsons_1/data/processed/forecast_train_{target}.csv"
    )
    ...
    drop_cols = [
        ...
        if col not in [f"{target}_0", f"{target}_{month_diff}"]
    ]
    ...
    x_train, x_valid, y_train, y_valid = train_test_split(
        df, df[target_mo], test_size=0.2, random_state=42
    )
...
if __name__ == "__main__":
                    ...
                    results.append(
                        {
                            ...
                            "R2": r,
                            "MAPE": m,
                            "Visit_Month_Diff": month_diff,
                        }
                    )
                    print("SMAPE: ", s, "R2: ", r, "MAPE: ", m, "\n")
                    # store the model
                    model_file = (
                        f"../models/model_{model_name}_{target}_{month_diff}.pkl"
                    )
    ...
    results_df.to_csv(
        f"~/parkinsons_proj_1/parkinsons_project/parkinsons_1/models/model_results/rfreg_{date}_results.csv"
    )
